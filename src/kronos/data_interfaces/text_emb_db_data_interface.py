...
def instantiate_client() -> WeaviateClient:  # type: ignore[no-any-unimported]
...
def get_collections_schema() -> List[CollectionSchema]:
    word_text_schema: PropertySchema = {
        "name": PropertyName.text.value,
        "description": "Text from which average word vector is derived",
        "data_type": DataType.TEXT,
    }
    ntype_schema: PropertySchema = {
        "name": PropertyName.ntype.value,
        "description": "Node type of the node of which text is an attribute",
        "data_type": DataType.TEXT,
    }
    compose_sent_schema: RefPropertySchema = {
        "name": RefName.composes_sent.value,
        "target_collection": CollectionName.sent.value,
    }
    word_collection: CollectionSchema = {
        "name": CollectionName.word.value,
        "description": "Text which is encoded with its average word vector",
        "properties": [word_text_schema, ntype_schema],
        "references": [compose_sent_schema],
    }

    sent_text_schema: PropertySchema = {
        "name": PropertyName.text.value,
        "description": "Text from which sentence transformer embedding is derived",
        "data_type": DataType.TEXT,
    }
    has_word_schema: RefPropertySchema = {
        "name": RefName.has_word.value,
        "target_collection": CollectionName.sent.value,
    }
    sent_collection: CollectionSchema = {
        "name": CollectionName.sent.value,
        "description": "Text which is encoded with its sentence transformer embedding",
        "properties": [sent_text_schema, ntype_schema],
        "references": [has_word_schema],
    }
